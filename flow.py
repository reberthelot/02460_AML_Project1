...
if __name__ == "__main__":
    ...
    # python 02460_week2_flow.py train --data tg --model "modeltg.pt" --device cuda --batch-size 10000 --epochs 3 --lr 1e-3
    # python 02460_week2_flow.py sample --data tg --model "modeltg.pt" --samples sample_flow_tg.png --device cuda
    # python 02460_week2_flow.py train --data cb --model "modelcb.pt" --device cuda --batch-size 10000 --epochs 3 --lr 1e-3
    # python 02460_week2_flow.py sample --data cb --model "modelcb.pt" --samples sample_flow_cb.png --device cuda
    # python 02460_week2_flow.py train --data mnist --model "modelmnist.pt" --device cuda --batch-size 10000 --epochs 15 --lr 1e-3
    # python 02460_week2_flow.py sample --data mnist --model "modelmnist.pt" --device cuda --samples sample_flow_mnist.png
    # python 02460_week2_flow.py train --data mnist --model "modelmnist.pt" --device cuda --batch-size 10000 --epochs 15 --lr 1e-3 --mask-type checkerboard
    # Parse arguments
    import argparse
    ...
    parser.add_argument('--mask-type', type=str, default='channelwise', choices=['checkerboard', 'channelwise','randominit'], help='type of mask to use in the coupling layers (default: %(default)s)')
    ...
    n_data = 10000000
    if args.data == 'mnist':
        toy = ToyData.MNIST(batch_size=args.batch_size)
    ...
    D = x_sample.shape[1] # Work on any size of data
    base = GaussianBase(D)

    # Define transformations
    transformations =[]
    
    # 5 for two Gaussians, 16 for chequerboard and 12 for mnist
    num_transformations = 12
    # 8 for tG and ch, 256 for mnist
    num_hidden = 256
    ...
    if args.mode == 'train':
        ...
        if args.data == 'mnist':
            ...
            model.eval()
            with torch.no_grad():
                samples = (model.sample((number_to_plot,))).cpu()
